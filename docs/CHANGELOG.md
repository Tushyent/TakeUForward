...
### Added
- **[SEO & GEO] LLMs.txt & LLM Full Protocol (`/llms.txt`, `/llms-full.txt`)**: Implemented the industry-standard `llms.txt` and `llms-full.txt` plain text manifests in `client/public/` for AI search engines and LLM crawlers (ClaudeBot, GPTBot, PerplexityBot, Google-Extended). Added alternate link tag in `index.html`.
- **[SEO & GEO] Dynamic Metadata Hook (`useSEO.js`)**: Created custom React `useSEO` hook to set page titles, meta descriptions, keywords, OpenGraph images, and canonical URLs dynamically on route changes.
- **[SEO & GEO] PWA Manifest (`manifest.json`)**: Added web app manifest with brand theme color (`#7C6AF7`), standalone display, and responsive app icons.
- **[SEO & GEO] Technical Audit Report (`docs/SEO_GEO_AUDIT.md`)**: Produced complete 27-part technical SEO, GEO, and AEO audit report.

### Changed
- **[Public Access] Public About Page (`/about`)**: Updated `axiosClient` and `AuthContext` so unauthenticated visitors on `/about` can view the page without auth redirects or 502 connection error overlays. Added a sticky public header bar with Sign In / Dashboard navigation.
- **[SEO & GEO] Extended Schema.org JSON-LD Graph**: Expanded `index.html` and `About.jsx` JSON-LD graph to include `WebSite` (with `SearchAction`), `EducationalOrganization`, `WebApplication`, `AboutPage`, `BreadcrumbList`, and `FAQPage` entities eligible for Google Rich Snippets & AI Overviews.
- **[SEO & GEO] Robots.txt & Sitemap Expansion**: Updated `robots.txt` and `sitemap.xml` with explicit allow rules for major search engines and AI crawlers.
- **[Security] Express Security Headers**: Configured Helmet and explicit HTTP headers in `server/index.js` (`X-Frame-Options`, `X-Content-Type-Options`, `Referrer-Policy`, `Permissions-Policy`).
- **[SEO & Web Visibility] Structured Data & Rich Meta**: Enriched `index.html` with Schema.org JSON-LD structured data (`WebSite`), canonical link tags, OpenGraph site name/locale, and Twitter Cards to maximize web discoverability and social share previews.
- **[Logging & Render Reliability] Production Debugging Logs**: Added structured Pino logging in `server/index.js` for rejected CORS origins, `server/routes/authRoutes.js` for OAuth callback processing and redirects, and `server/config/passport.js` for Google OAuth user verification and creation events.
- **[Sitemap] Sitemap Expansion**: Updated `client/public/sitemap.xml` with current timestamp `<lastmod>` and indexed public pages (`/`, `/login`, `/about`, `/alumni-invite`, `/support`).
